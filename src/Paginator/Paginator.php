...
class Paginator
{
     ...
     * @return $this
     */
    public static function paginate(Query $query, $current_page = 1, $items_on_page = self::MIN_ITEMS)
    {
        $total_items = count($query->getResult());

        $items_on_page = $items_on_page > $total_items ? $total_items : $items_on_page;
        $items_on_page = $items_on_page > 0 ? $items_on_page : self::MIN_ITEMS;

        $pages_count = ceil($total_items / $items_on_page);

        $items = $query
            ->setSkip($items_on_page * ($current_page - 1))
